...
export default function FinalGamePage() {
  ...
  const tvWarmRef = useRef<HTMLDivElement | null>(null);
  useEffect(() => {
    const t1 = setTimeout(() => setWarmup(false), 900);
    const t2 = setTimeout(() => setWarmup(false), 2200);
    const end = () => setWarmup(false);
    const pointer = () => setWarmup(false);
    tvWarmRef.current?.addEventListener("animationend", end);
    curtainRef.current?.addEventListener("animationend", end);
    window.addEventListener("pointerdown", pointer, { once: true });
    window.addEventListener("keydown", pointer, { once: true });
    return () => {
      clearTimeout(t1);
      clearTimeout(t2);
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        {warmup && <div ref={tvWarmRef} className="tv-warmup" />}
      </div>
